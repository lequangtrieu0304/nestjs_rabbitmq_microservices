...
export class TokensController {
  ...
  public async decode(
    ...
    if (!data?.token) throw new BadRequestException('Chưa đăng nhập.');
    const tokenData = await this.tokensService.decode(data.token);
    return {
      status: tokenData?.userId ? HttpStatus.OK : HttpStatus.UNAUTHORIZED,
      message: tokenData?.userId ? 'DECODE_SUCCESS' : 'DECODE_FAILED',
      data: tokenData,
    };
  }
